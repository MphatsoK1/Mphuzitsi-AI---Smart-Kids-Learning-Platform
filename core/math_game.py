...
def generate_math_problem(level):
    """Generate a math problem based on level configuration"""
...
def get_math_level(request):
    """Get math problems for a specific level"""
    ...
    try:
        ...
        for _ in range(level.problems_required):
            problem = generate_math_problem(level_number)
...
def get_next_math_level(request):
    """Get next math level information"""
